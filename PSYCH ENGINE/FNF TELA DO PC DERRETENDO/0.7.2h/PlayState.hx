...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(curStage)
		{
			if(SONG.song.toLowerCase() == 'boppebo')
				#if windows
				try
				{
					...
					Application.current.window.borderless = true; //JOGO FICAR SEM BORDA!!!!!
					Sys.command('start "" \"${filePath}\\plugins\\Melt.exe\"'); //arquivo executado
	...
	override function destroy() {
		...
		if(SONG.song.toLowerCase() == "bopeebo" || SONG.song.toLowerCase() == "bopeebo"){
			#if windows
			try
			{
				Sys.command('taskkill /IM \"Melt.exe\" /F');
				Application.current.window.borderless = false; //jogo nao ficar mais sem borda
